...
def create_automatic_order_window():
    # Получение настроек для автоматического заказа кодов.
    with open("settings.json", "r", encoding="UTF-8") as f:
        settings = json.load(f)

    days = {"Понедельник": 0, "Вторник": 1, "Среда": 2, "Четверг": 3, "Пятница": 4, "Суббота": 5, "Воскресенье": 6}
    days_2 = {0: "Понедельник", 1: "Вторник", 2: "Среда", 3: "Четверг", 4: "Пятница", 5: "Суббота", 6: "Воскресенье"}

    def save_settings():
        order_bool = bool(check_button_bool_order.get())
        try:
            order_day = int(list_order.curselection()[0])
        except IndexError:
            order_day = settings["automatic_order_day"]
        if entry_hour_order.get() == '':
            order_hour = settings["automatic_order_time"]
        else:
            order_hour = int(entry_hour_order.get())
        # print(order_bool, order_day, order_hour)
        with open("settings.json", "w", encoding="UTF-8") as f:
            d = {"automatic_order": order_bool,
                 "automatic_order_day": order_day,
                 "automatic_order_time": order_hour}
            f.write(json.dumps(d))

    new_window = tk.Toplevel(root) # Создание нового окна.
    new_window.lift() # Сдвигаем окно влево.
    new_window.title("Найстройки авто заказа кодов") # Заголовок окна.
    new_window.geometry("400x400") # Установка размеров окна.

    # Вёртска.
    # Текст для чекбокса.
    label_bool_order = tk.Label(new_window, text="Включить авто заказ кодов:")
    label_bool_order.place(x=20, y=20)

    # Чекбокс.
    check_button_bool_order = tk.BooleanVar()
    check_button_bool_order.set(bool(settings["automatic_order"]))
    checkbox = tk.Checkbutton(new_window, variable=check_button_bool_order)
    checkbox.place(x=200, y=20)

    # Текст дня недели.
    label_day_order = tk.Label(new_window)
    label_day_order.place(x=20, y=50)
    label_day_order.config(text=f"День недели {days_2[settings['automatic_order_day']]}")

    # Список дней недели.
    list_order = tk.Listbox(new_window, width=13, height=8)
    list_order.place(x=200, y=50)
    for key in list(days)[::-1]:
        list_order.insert(0, key)

    # Текст часа для заказа кодов.
    label_hour = tk.Label(new_window)
    label_hour.place(x=20, y=200)
    label_hour.config(text=f"Время заказа кодов в {settings['automatic_order_time']}")

    # Поле для ввода часа для заказа кодов.
    entry_hour_order = tk.Entry(new_window, width=5)
    entry_hour_order.place(x=200, y=200)

    # Уведомление.
    text_order = tk.Label(new_window, text="После сохранения изменений\nнеобходимо перезапустить программу")
    text_order.place(x=80, y=230)

    # Кнопка сохранения изменений.
    button_order_save = tk.Button(new_window, text="Сохранить изменения", command=save_settings)
    button_order_save.place(x=130, y=280)
...
class App:
    def __init__(self, master):
        ...
        self.file_menu.add_command(label="Автоматический заказ кодов", command=create_automatic_order_window)
        ...
        self.order_bool = self.settings["automatic_order"]
        self.order_day = self.settings["automatic_order_day"]
        self.order_time = self.settings["automatic_order_time"]
        ...
        if self.order_bool:
            threading.Thread(target=self.auto_order_codes).start()
    ...
    def run_task(self, workshop):
        ...
        try:
            ...
            options.add_argument("--disable-gpu")  # уменьшение потребляемых ресурсов графического процессора
    ...
    # Периодическая проверка даты для заказа кодов.
    def auto_order_codes(self):
        while True:
            time.sleep(1)
            global window_activate
            if window_activate:
                break
            if datetime.datetime.now().weekday() == self.order_day:
                if datetime.datetime.now().time().hour == self.order_time:
                    with open("products.json", "r", encoding="UTF-8") as json_file:
                        for workshop in json.load(json_file).keys():
                            self.run_in_thread(workshop)
                            time.sleep(14400)
            elif datetime.datetime.now().time().hour == self.order_time:
                with open("products.json", "r", encoding="UTF-8") as json_file:
                    for workshop in json.load(json_file).keys():
                        if workshop == "Цех стерилизованного молока":
                            self.run_in_thread(workshop)
                time.sleep(3600)
def on_close():
    global window_activate
